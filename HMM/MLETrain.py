...
from utils.params import LAMBDAS
...
class MleExtractor:
    def __init__(self, e_mle_file, q_mle_file, extra_file):
        ...
        self._transition_lambdas = LAMBDAS
    ...
    def prob_func(self, word_sequence, curr_word_idx, src_prob_row, prev_POS, curr_POS, log=True):
        ...
        if log:
            if np.std(src_prob_row) > 0:
                good_chance = list(np.argsort(src_prob_row)[-8:])
            else:
...
if __name__ == "__main__":
    if len(sys.argv) < 4:
        print("input\t\tMLETrain:\tinput_file_name,\t q.mle,\t e.mle\n\n")
